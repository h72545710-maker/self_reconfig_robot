...
$master = Start-Process -FilePath (Join-Path $BinDir "master_node.exe") `
    ...
    -RedirectStandardOutput "$LogDir\offline_master.log" `
    -RedirectStandardError "$LogDir\offline_master.err" `
...
$module1 = Start-Process -FilePath (Join-Path $BinDir "module_node.exe") `
    ...
    -RedirectStandardOutput "$LogDir\offline_module1.log" `
    -RedirectStandardError "$LogDir\offline_module1.err" `
    -PassThru -WindowStyle Hidden

$module2 = Start-Process -FilePath (Join-Path $BinDir "module_node.exe") `
    -ArgumentList "--id", "2", "--battery", "88", "--x", "1", "--y", "2", "--duration", "$($Duration - 2)" `
    -RedirectStandardOutput "$LogDir\offline_module2.log" `
    -RedirectStandardError "$LogDir\offline_module2.err" `
    -PassThru -WindowStyle Hidden

$module3 = Start-Process -FilePath (Join-Path $BinDir "module_node.exe") `
    -ArgumentList "--id", "3", "--battery", "80", "--x", "1", "--y", "3", "--duration", "$($Duration - 2)" `
    -RedirectStandardOutput "$LogDir\offline_module3.log" `
    -RedirectStandardError "$LogDir\offline_module3.err" `
    -PassThru -WindowStyle Hidden

Wait-Process -Id $master.Id, $module1.Id, $module2.Id, $module3.Id

Write-Host "`n===== master output ====="
Get-Content "$LogDir\offline_master.log"

Write-Host "`n===== module output ====="
Get-Content "$LogDir\offline_module1.log"
Get-Content "$LogDir\offline_module2.log"
Get-Content "$LogDir\offline_module3.log"
